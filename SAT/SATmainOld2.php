...
<div class="container">
  ...
  <section class="main">
    ...
    <div class="col" id="buttons">
        ...
        <i class="fa fa-angle-left icons iconR" id="back" title="back" style="font-size:36px; color:#1672ce"></i>
        <i class="fa fa-angle-right icons" id="nextWord"  title="next" style="font-size:36px; color:#1672ce"></i>
        <i class="fa fa-file-text-o icons" id="flip"  title= "show definition" style="font-size:36px; color:#1672ce"></i>
        <i class="fa fa-check-square-o icons"  id="mem" title="save word to 'memorized' list" style="font-size:36px; color:#1672ce">
    ...
    <div class="col" id="progress">
      <div class="progress">
      <div class="progress-bar" id="progress-bar" role="progressbar" aria-valuenow="70" aria-valuemin="0" aria-valuemax="100" style="width:70%"></div>
    </div>
  </section>
  <section class="quiz">
...
$(document).ready(function(){
  ...
  //  updateProgress(numMemWordsUrl);
    ...
    $("#back").click(function(){
      console.log("back pressed", window.seenWords.length);
      back();
    });
...
function nextWord(){
  ...
  window.seenWords.push(rand);
  console.log("seenWords array", seenWords);
...
function back(){
  try{
    var lastWord = window.seenWords.pop();
    $("#word").html(lastWord["word"]);
    $("#def").html(lastWord["definition"]);
    $("#def").css("visibility", "hidden");
    window.hash=lastWord["hash"];
  }
  catch(err){
    console.log("seenWords array empty");
  }
}

function updateProgress(){

  url = "http://localhost/SAT/checkProgress.php?user=a@a.com";
  console.log("update Progress", url);
  $.get(url, function(data, status) {
    console.log("Progress data", data);
    js =  JSON.parse(data);
     console.log("prog", js);
     var progress = js["count"];
...
function loadWords(url){
    ...
    // ajax call to url, returns data produced by getVocab.php
    $.get(url, function(data, status){  
            // alert("Data: " + data + "\nStatus: " + status);
            console.log("loadwords url, data.length", url, data.length);
        ...
        console.log("array length",wordsArray.length);  
...
function memorizeWord(){
  // check that word is not alreayd in mem list
  confirm("save this word to list of memorized words");
